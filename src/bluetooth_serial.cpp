...
static void* _spp_rx_queue = NULL;
static void* _spp_tx_queue = NULL;
static void* _spp_tx_done = NULL;
static void* _spp_task_handle = NULL;
static void* _spp_event_group = NULL;
...
bool BluetoothSerial::begin(const char* localName, bool isMaster)
{
    _isMaster = isMaster;
    //if (localName.length()){
    //    local_name = localName;
    //}
    return _init_bt(localName/*.c_str()*/);
}
...
char BluetoothSerial::peek(void)
{
    char c;
...
int BluetoothSerial::read(char* buffer, int size)
{
    if (!_spp_rx_queue) return 0;
    int n = 0;
    char c;
    while (n != size && xQueueReceive(_spp_rx_queue, &c, 0))
    {
        buffer[n]=c;
        n++;
    }
...
int BluetoothSerial::write(char c)
{
    return write(&c, 1);
}

int BluetoothSerial::write(const char *buffer, int size)
{
    if (!_spp_client){
        return 0;
    }
...
bool BluetoothSerial::connect(const char* remoteName)
{
